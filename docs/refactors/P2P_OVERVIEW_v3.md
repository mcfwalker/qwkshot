...
**Status:** Reflects the architecture after completing the core Phase 3 refinements of the Assistants API refactor plan.
...
7.  The Scene Interpreter processes each step in the `MotionPlan`:
    ...
    *   It enforces constraints (like not colliding with the model bounding box or exceeding maximum camera distance) during calculation.
    *   It determines appropriate easing based on parameters (`speed`, `easing`).
8.  The Interpreter outputs a final list of frame-by-frame **`CameraCommand[]`** objects (each containing position, target, duration, easing).
9.  These commands are sent to the frontend client.
10. A dedicated **AnimationController** component within the React Three Fiber render loop reads these commands and smoothly interpolates the Three.js camera frame by frame, producing the final animation.
...
*   **Scene Interpreter:** (`src/features/p2p/scene-interpreter/`) **(Major Refactor/Rewrite)**
    ...
    *   For each step:
        ...
        *   Applies constraints (height, distance, bounding box via raycasting) during calculation.
        *   Determines appropriate easing function (using `d3-ease`) based on parameters.
        *   Handles duration allocation.
    *   Outputs an array of `CameraCommand` objects.
*   **Animation Controller (Client):** (`src/components/viewer/AnimationController.tsx`) **(Largely Unchanged Conceptually)**
    ...
    *   Interpolates the camera's `position` and `target` between command states over the specified `duration`.
