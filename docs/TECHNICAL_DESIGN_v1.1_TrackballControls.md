...
```markdown
# Modern 3D Viewer - Technical Design Document (v1.1 - TrackballControls & Roll)
...
### 1.7. 3D Rendering
- **Library:** Three.js
- **Integration:** Using `@react-three/fiber` and `@react-three/drei`.
- **Interactive Controls:** Uses `<TrackballControls>` from `@react-three/drei` for user setup phase (enabled when scene is unlocked and animation is not playing). Allows free-form rotation/tumbling.
- **Animation Execution:** A dedicated `AnimationController` component uses the `useFrame` loop to directly manipulate the `camera.position` and `camera.quaternion` during playback (when scene is locked/interactive controls disabled).
...
### 4.2 API Routes
- **Purpose:** Handles camera path generation (`/api/camera-path`), system info/health checks, authentication callbacks.
- **`/api/camera-path`:**
    - Receives prompt, duration, modelId.
    - Fetches context (`SceneAnalysis`, `EnvironmentalMetadata`, `initialCameraState` including a `THREE.Camera` instance) via `MetadataManager`.
    - Calls `OpenAIAssistantAdapter.generatePlan` to get `MotionPlan`.
    - Calls `SceneInterpreter.interpretPath` with plan and context.
    - **Serializes `CameraCommand[]` response, including the optional `orientation` field** (as `{x, y, z, w}` or `null`).
...
- **Overview:** Translates user prompts into camera commands. Uses OpenAI Assistants API for planning and a deterministic Scene Interpreter using local context (`SceneAnalysis`, `EnvironmentalAnalysis` - including `userVerticalAdjustment` and initial `Camera` state) for execution.
- **Client Interaction:**
    - **Setup:** User positions camera using `<TrackballControls>`.
    - **Environment Saving:** Client (`CameraAnimationSystem`) calls `updateEnvironmentalMetadataAction` Server Action on lock, passing current camera state (`position`, `target`, `fov`) and `userVerticalAdjustment`.
    - **Path Generation:** Client (`CameraAnimationSystem`) calls `/api/camera-path`. **Response is deserialized**, converting `position`, `target` (Vector3), and `orientation` (Quaternion) back to THREE objects.
- **Backend Flow (`/api/camera-path`):**
    ...
    9. Calls `interpreter.interpretPath(motionPlan, sceneAnalysis, environmentalAnalysis, initialCameraState)`. The interpreter:
        ...
        - **Generates `CameraCommand[]` keyframes, calculating and adding `orientation: THREE.Quaternion` for `rotate` steps with `axis: 'roll'`.**
    ...
    12. Returns final **serialized `CameraCommand[]`** (including orientation) to client.
...
- **Keyboard Navigation:** Ensure all interactive elements are reachable and operable via keyboard (Tab, Shift+Tab, Enter, Space). **Viewport panning is supported via arrow keys/WASD (using `CameraMover` component). Dedicated keyboard controls for trackball rotation/zoom are not currently implemented.** 
- **Screen Reader Support:** Provide descriptive text alternatives for non-text content (e.g., `alt` text for informative images, ARIA labels). Use `sr-only` class for visually hidden text beneficial to screen readers.
- **Color Contrast & Themes:** Ensure sufficient contrast ratios (AA standard minimum). Test themes (light, dark, high-contrast) for accessibility.
